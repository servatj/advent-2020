...
const validPosition = (letter, position, password) => {
  return password[Number(position) - 1] === letter
} 
...
lineReader.on("line", async (line) => {
  ...
  try {
    ...
    if((validPosition(letter, low, password) && !validPosition(letter, high, password)) || (!validPosition(letter, low, password) && validPosition(letter, high, password))) {
      valids++
    }
